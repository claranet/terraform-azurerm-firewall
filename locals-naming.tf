locals {
  ...
  firewall_name  = coalesce(var.custom_firewall_name, data.azurecaf_name.firewall.name)
  public_ip_name = coalesce(var.public_ip_custom_name, data.azurecaf_name.firewall_pip.name)
